...
- **Update** — `resolution-runtime-reports-alternatives` landed: a resolution now reports the runners-up it rejected, so the reviewed *use this listing instead* control has something to offer. New `src/alternatives.ts` (the floor filter, the cap, the barcode state and the derived reason), `ranking.ts` gained `scoreBreakdown` with `scoreCandidate` delegating to it, and `Resolution` gained `alternatives`. New concepts: `capabilities/resolution-alternatives` (7 scenarios), `qa/resolution-alternatives` (17 checks). **331 tests, up from 300**, and **12 mutations each shown to turn exactly the right check red**. Gate green: lint 0, typecheck 0, build 0, 21 files / 331 tests, `okf:check` 0.

- **Learning** — 🔴 **"Report the runners-up" reads as *all of them*, and that would have been wrong.** Every alternative gets a **swap button** on the reviewed screen, so a candidate below `MIN_EVIDENCE_TO_REPORT` is not a runner-up — it is the $3,727-server-for-a-$13-accessory mistake with a control on it, and a wrong pairing is indistinguishable from a real one downstream. Both gates apply, the same two the chosen candidate had to pass: the numeric floor **and** `hasPartNumberEvidence`, because brand + first-party + in-stock totals exactly the floor on its own. 🔑 **The row's most important decision is a refusal**, and it came from reading the floor's own docblock rather than from the criterion.

- **Learning** — 🔴 **The 53-row replay does NOT pin the score, and my own checklist claimed it did.** A mutation adding `score += 1` to the `brandMatch` credit left the row-by-row check **green**: the replay compares *outcomes*, and a uniform nudge moves no candidate past another, so it changes which listing wins for none of those rows. 🔑 **A checklist entry naming the wrong check as a guard is worse than no entry**, because the next reader stops looking. Corrected in place, and the arithmetic is now pinned by `ranking-breakdown.spec.ts` — asserting against `WEIGHTS` **by name**, so a deliberate reweighting stays correct through it while an unnamed addition does not.

- **Learning** — 🔴 **A second mutation found a check asserting a tautology.** Deleting the line that records what each probe published left the corpus check green, because it asserted only that `barcodeState` was one of three values — and `unprobed` is one of three. 🔑 **Fixed by measuring first, then asserting**: the corpus carries **71 alternatives across 18 rows — 12 `disagreed`, 6 `absent`, 53 `unprobed`**, so all three states genuinely occur and all three can be required. ⚠️ Asserted as *at least one of each* rather than the exact counts, because the counts are a property of this corpus and would make a legitimate ranking change look like a regression.

- **Learning** — 🔑 **An alternative has to say whether anybody LOOKED, and that is three states rather than a nullable barcode.** Probes are the expensive part — 2.70 per row against a ceiling of eight — so most ranked candidates are scored from search-result text and never opened. `null` would collapse "we opened it and it published none" into "nobody opened it", and those are different risks: a reviewer swapping is choosing which to take on. The same distinction the outcomes already draw between `unconfirmed` and `unverifiable`, where 91 of 255 captured pages publish no barcode at all.

- **Learning** — ⚠️ **The reason a candidate ranked lower is derived from the signals, never from the score.** The weights are ordinal and unfitted — `ranking.ts` says so itself — so "scored eight lower" is noise dressed as information. `reasonRankedLower` names the strongest signal the chosen candidate had that this one lacks, and every one of those is something a reviewer can check against the page in front of them. 🔑 **And a tie says it was a tie**: when no signal differs, discovery order decided it, and claiming a difference the scorer never made is worse than admitting there is none.

- **Learning** — 🔑 **`scoreCandidate` delegates to `scoreBreakdown` rather than the breakdown re-deriving what the scorer did.** Two implementations of that arithmetic is how a signal list comes to disagree with the number it explains. ⚠️ **And the two facts that look contradictory are now checked together**: `queryToken` fires once per hit and the score counts every one, while the signal list is a set and names it once — the count ranks a candidate, the name explains it.

- **Learning** — ⚠️ **`noUncheckedIndexedAccess` turned three spec assertions into type errors, and the fix was an assertion rather than a cast.** `out[0]!` would have silenced the compiler while turning an empty list — the exact defect several of those checks are about — into a confusing runtime error instead of a clear failure. A `first(list, what)` helper that throws names what was missing.
...
- **Learning** — 🔴 **This runtime cannot take a screenshot, and the reason is structural rather than a missing dependency.** The row is named for one; what is deliverable is the page's **bytes**. The proxy exists because the retailer refuses a plain request, so a headless browser inside a Kubernetes Job would make exactly the request the proxy avoids and fail on precisely the sites this runtime is for. Routing a browser through a proxy is a different vendor product with its own credential — and `BRIGHTDATA_API_KEY`, the credential this runtime's own fetch path needs, **is set in no workspace repo 🔴 (this entry said “not set anywhere on this machine” and that was false — it measured the workspace and called the result the machine; the PO's own spike repo carried the key and its zone all along, and a live run on 2026-09-11 used them)**, so the alternative could not even be probed. 🔑 **The bytes are the better artefact for one job and useless for the other**: they prove byte-for-byte what the parse read and can be re-parsed to settle a matcher dispute, and they show a person nothing. So `png` is in the vocabulary, no code path emits it, a job asking for it is **refused by name**, and the portal's `thumbnailUrl` will be null for every capture — priced as its own row rather than hidden.
